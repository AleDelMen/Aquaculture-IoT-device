...
#the "global_conf_EU433.json" may need change to your need.
#Put the configuration file on the specified path

#Verify  if the Packet Forwarder is running and check if there are not errors.
sudo systemctl status lrgateway

#Start packet forwarder on boot.
